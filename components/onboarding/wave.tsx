import { HEIGHT, MIN_LENGTH, Side, WIDTH } from '@/configs/constants';
...
export default function Wave({
  ...
  const R = useDerivedValue(() => {
    const value = Math.min(position.x.value - MIN_LENGTH, WIDTH / 2.5);
    return value > 0 ? value : MIN_LENGTH;
  });
  const ledge = useDerivedValue(() => {
    const baseLedge = Math.max(0, position.x.value - MIN_LENGTH - R.value);
  ...
  const animatedProps = useAnimatedProps(() => {
    const stepY = position.x.value - MIN_LENGTH;
  ...
  const maskElement = (
    ...
    >
      <AnimatedPath
        fill={
          Platform.OS === "android"
            ? (children.props as any).slide.color
            : (children.props as any).color
        }
